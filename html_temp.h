...
"<title>Controle de Temperatura</title><style>"
"*{margin:0;padding:0;box-sizing:border-box}"
"body{font-family:system-ui,-apple-system,sans-serif;background:linear-gradient(135deg,#ff6a6a 0%,#ff4500 100%);color:#fff;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}"
...
".water{position:absolute;bottom:0;width:100%;background:linear-gradient(180deg,#ff8a65,#ff5722);transition:height 0.8s;border-radius:0 0 13px 13px}"
...
".limit{position:absolute;width:100%;height:2px;background:#ffc107;left:0;z-index:5;transition:bottom 0.8s ease}"
".limit::after{content:attr(data-label);position:absolute;right:5px;top:-10px;transform:translateY(-50%);font-size:0.8rem;font-weight:bold;color:#ffc107;text-shadow:0 1px 3px rgba(0,0,0,0.5)}"
...
"<h1 class=\"title\">🔥 Controle de Temperatura</h1>"
"<div class=\"tank\">"
"<div class=\"level-text\" id=\"level\">--°C</div>"
"<div class=\"water\" id=\"water\"></div>"
"<div class=\"limit\" id=\"max-limit\" data-label=\"MAX: --°C\"></div>"
"<div class=\"limit\" id=\"min-limit\" data-label=\"MIN: --°C\"></div>"
"</div>"
"<form class=\"form\" id=\"limits-form\">"
"<div class=\"input-group\"><label>Temperatura Mínima (°C):</label>"
"<input type=\"number\" id=\"min-input\" required></div>"
"<div class=\"input-group\"><label>Temperatura Máxima (°C):</label>"
...
"const nivel=data.nivel_atual_Temp;"
"water.style.height=(nivel/50*100)+'%';"
"levelText.textContent=nivel+'°C';"
"maxLimit.style.bottom=(data.max/50*100)+'%';"
"minLimit.style.bottom=(data.min/50*100)+'%';"
"maxLimit.setAttribute('data-label','MAX: '+data.max+'°C');"
"minLimit.setAttribute('data-label','MIN: '+data.min+'°C');"
...
"if(minVal>=maxVal){showFeedback('A temperatura mínima deve ser menor que a máxima.','#e74c3c');return;}"
"try{"
"const res=await fetch('/api/limites',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`min=${minVal}&max=${maxVal}`});"
"if(res.ok){showFeedback('Limites atualizados!','#2ecc71');await fetchData();}else{throw new Error('Failed to update');}}"
"catch(err){console.error('Update failed:',err);showFeedback('Erro ao enviar dados.','#e74c3c');}});"
